...
- 監査日: 2026-07-24（文献・刺激監査は2026-07-17、統合版5.4.0で未署名遠隔リンク、GitHub Pages配備、3段階の研究者設定UIを追加）
...
11. 同一端末の監督下実施 `supervised` と、参加者がZIPを外部portalへ手動返却する `remote_manual_upload` を区別します。version 5.4.0は利便性を優先し、schema 3の未署名リンクをproduction／stagingでも発行・実行できるようにしました。リンクは再利用可能で、発行者・参加者の認証や一回限りの招待ではありません。portal受領とZIP照合は研究側が別途実施します。
...
14. 凍結 `deployment-config.json` がproduction／stagingのexact HTTPS origin、public participant base URL、return portal origin allowlist、研究者UI可否を決めます。現行のGitHub Pages設定は同originで `supervised` と未署名 `remote_manual_upload` を許可します。研究者UIも公開となるため、認証済み設定を必要とする研究は専用originへ配備してください。任意HTTPS copyはblockし、loopbackは明示的test sessionだけを許可します。
...
研究者設定画面は「研究情報」「課題と手続き」「配布または開始」の3段階に分けます。研究ID、条件ID、実施site ID、配布ID、参加者向け研究名、実施機関、同意文書版、予定所要時間、同意説明URL、研究者連絡先URLを必須とし、形式違反は該当欄の直下に理由を表示します。各IDはASCII英字で始まる1–64文字、同意文書版は許可文字による1–64文字、所要時間は1–240分です。外部URLはcredentialと表示制御文字を含まないHTTPSに限定し、各URLは1,024文字以内、参加者用リンク全体は4,096文字以内とします。consent/contact/return URLのfragmentは正規化後も保持します。遠隔参加者用リンクを作成する場合だけ、研究データ用に承認され受領証を発行できる外部HTTPS返却portalも必須です。これらの値は直接識別子を入力する欄ではありません。

bare URLは英語が既定です。参加者用リンクの言語selectorは研究者UIの現在言語に初期同期し、研究者が変更した後は独立した選択として保持します。生成した案内文もその参加者開始言語を使用します。production／stagingの承認済みoriginでは同じbrowser／device上の `supervised` と、未署名リンクによる `remote_manual_upload` を開始できます。loopbackでは両手順をTEST sessionとして検証できます。リンクは `link_version`、`battery_version`、`protocol` とその版、`catalog_sha256`、binding先の `stimulus_set` と `manifest_sha256`、選択課題、feedback、開始言語、および上記study metadataを固定します。URLに参加者codeや結果は含みませんが、研究名、条件、protocol、課題名、外部URL等をqueryへ含むため、URLや公開source codeを用いた盲検化機構ではありません。

参加者用リンクschema 3はfail closedで検査します。必須値の欠損・重複、未知または非対応の値、非canonicalな順序・URL、旧版、deployment ID／config SHA／session type／current originの不一致、課題scopeの逸脱、metadata形式違反、catalog・set・manifest・protocol bindingの不一致、および変更によってこれらの条件を満たさなくなったリンクは、研究者設定の既定値へfallbackせずinvalid-link画面で停止します。queryなし、または単一の対応言語だけを指定したURLだけが、configで許可されたresearcher originの研究者設定入口として有効です。production／stagingの承認済みoriginは `session_type=research` のリンクを生成・実行し、loopbackは `session_type=test` に固定します。

schema 3の静的URLに暗号署名や認証はありません。リンクは再利用可能であり、本人確認、一回限りの招待、別の有効構成への置換検知、研究者画面のaccess controlを提供しません。現行runtimeはこの制約を明示したうえで、承認済みproduction／staging originでも未署名linkを受理します。実課題ID、condition、protocol等もURLと公開sourceから判別できるため、盲検化は別途設計します。外部portalへの認証・upload・retry・duplicate処理・retention・受領証はこのstatic siteの外にあり、アプリはportalがZIPを受領したか検証できません。`remote_manual_upload` では同意確認前から「ZIPを保存 → 承認済みportalへ手動upload → portalの受領証を保持 → browser copyを明示消去」と表示します。

参加者codeは入力時にuppercaseへ正規化し、ASCII英字で始まる1–32文字のASCII英字・数字・hyphen・underscoreに限定します。氏名、email address、学籍番号、健康情報などの直接識別情報ではなく、研究用の仮名codeを割り当てます。研究者画面はランダムcodeを提案し、参加者開始言語に合わせたリンク・code・返却手順の案内文をまとめてcopyできますが、承認済み対応表への記録と重複防止は研究者が担います。codeは再利用可能URLには埋め込みません。codeは選択課題の決定論的shuffleをseedし、開始前overviewで確認・修正できます。参加者向け課題名は、このshuffle後の実施位置に従う「リスニング課題1、2…」です。番号とpitch／formant／duration／rise-timeの間に固定mappingはありません。
...
- 統合版: `battery_version = 5.4.0`、`app_build_id = audio-discrimination-5.4.0`
...
- UI開始言語: bare URLは英語。参加者用リンク言語selectorは研究者UI言語に初期同期し、変更後は独立して保持
- 参加者label: shuffle後の順番に従うneutralな連番。課題typeへの固定mappingなし
- 実施mode: production／stagingで同一browser/deviceの `supervised` と、未署名リンクからZIPを外部portalへ手動返却する `remote_manual_upload` を許可。loopbackはTEST専用
...
`.github/workflows/ci.yml` はJavaScript syntax、UI/result-bundle contract、ZIP digest、親manifest、runtime registry、3 profileの音響条件、pinned Praat 6.4.19による再構成metadataを検査します。release時は [RELEASE_CHECKLIST.md](RELEASE_CHECKLIST.md) に従い、exact commit/tag、CI run、production origin、served asset-set／script SHA、approved protocol／consent、browser/device UAT、選択mode、known limitations、実配布link、外部portalのend-to-end receipt、rollback targetを保存します。artifact／rightsの現状は [NOTICE.md](NOTICE.md) に分離して記録します。CI合格は倫理承認、participant UAT、外部portal検証、音声再配布許諾を意味しません。

`deployment-config.json` はsecretを含まない凍結release入力です。production／stagingではconfigに記載したexact originだけが `supervised` または `remote_manual_upload` research sessionを開始できます。return URLはexact origin allowlistまたは明示的な `"*"` wildcardに従い、現行のGitHub Pages設定は任意のHTTPS返却先を許可します。研究者UIはconfigで全体を無効化でき、許可時もexact researcher originでだけ表示します。任意HTTPS copyと `file://` はblocked、loopbackは画面・link・checkpoint・CSV・manifest・filenameで識別されるtest sessionだけです。GitHub Pagesの研究者UIは公開で、未署名リンクは再利用可能です。base URLではなく実配布linkそのものをUAT・archiveし、返却ZIPとreceiptを外部で照合します。
