...
- **Remote manual return (`administration_mode = remote_manual_upload`)**: the researcher creates a reusable participant link, the participant downloads the ZIP, uploads it through the configured external HTTPS return portal, and retains the portal's receipt. The app itself never uploads or verifies acceptance. Production links are intentionally unsigned: they do not authenticate the researcher or participant, are not one-time invitations, and must be distributed and reconciled under the study's own procedures.

This repository is a static site. It cannot authenticate invitations, make a link one-time, authenticate a researcher, provide a data-receiving backend, or verify a portal receipt. For remote studies, the receiving service, access controls, retention rules, duplicate handling, participant support, and receipt reconciliation must be supplied outside this app. Runtime authority comes from the frozen, non-secret [deployment-config.json](deployment-config.json). The checked-in GitHub Pages configuration enables both supervised sessions and unsigned remote manual-return links on the configured origin. See [PRODUCTION_DEPLOYMENT.md](PRODUCTION_DEPLOYMENT.md) for the resulting trust boundary and deployment procedure.
...
1. Open the deployment on the exact HTTPS origin frozen in its deployment configuration. A bare URL starts in English. Arbitrary HTTPS copies and `file://` cannot run research sessions. `localhost`, `127.0.0.1`, and `[::1]` run an explicitly labelled `test` session when local tests are enabled in the configuration.
2. Follow the three-step researcher wizard. First, enter the required non-participant study metadata: study, condition, site, and distribution IDs; participant-facing title; institution; consent version; expected minutes; and external HTTPS consent and contact URLs. Invalid fields are identified next to the input. Do not enter direct identifiers or credentials in these fields or URLs.
3. On the second step, select one or more available tasks, a study profile (procedure plus its bound stimulus set), and feedback mode.
4. On the third step, start a supervised session on the current device, or create an unsigned participant link for remote manual return. A remote link requires an approved HTTPS result-return portal. Its participant-language selector initially follows the researcher UI language and remains independently selectable. After creating the link, assign or generate a pseudonymous participant code and copy the complete invitation containing the link, code, and completion instructions. Verify every encoded setting, assigned code, and return-portal address before sharing. Treat the URL as a reusable public study configuration, not as proof of identity or a one-time invitation. A local loopback environment creates only visibly labelled TEST links.
...
The researcher screen creates participant links on an enabled production/staging researcher origin and creates visibly labelled TEST links on loopback. A link pins participant-link schema 3, battery version, deployment ID, frozen deployment-config SHA-256, `session_type`, protocol and protocol version, stimulus-catalog hash, bound stimulus-set ID and manifest hash, selected tasks, feedback mode, starting language, study/condition/site/distribution IDs, participant-facing study title, institution, consent version, expected duration, and the three external URLs. The participant-language selector initially follows the researcher UI language; changing it makes that choice independent. The generated invitation text uses the participant language selected for that link. The URL contains neither a participant code nor results, but it is reusable, unsigned, and exposes the named study configuration.

Participant-link parsing remains fail closed for structure and reproducibility. A missing, duplicate, unknown, unsupported, outdated, non-canonical, or internally inconsistent value—including deployment/session type, current origin, protocol, task, metadata, HTTPS URL, catalog, set, or manifest mismatch—stops at an invalid-link screen instead of silently falling back to current defaults. Consent, contact, and return URLs must use HTTPS, contain no credentials or display-control characters, and are limited to 1,024 characters each. Return URLs must match an exact origin in `allowed_return_url_origins` unless that list explicitly contains the `"*"` wildcard; the checked-in GitHub Pages configuration uses this wildcard to accept any HTTPS return portal. URL fragments are preserved. The complete participant link is limited to 4,096 characters. Older link schemas fail closed. These checks detect accidental or casual alteration but do not authenticate the issuer because schema 3 carries no digital signature.

Participant codes are normalized to uppercase and must contain 1–32 ASCII letters, digits, hyphens, or underscores, beginning with an ASCII letter. Use a pseudonymous study code; do not enter a name, email address, student number, health information, or other directly identifying information. The researcher wizard can suggest a random code for an invitation, but the researcher remains responsible for recording it in the approved study roster and preventing duplicates. The code is delivered in the invitation text rather than encoded in the reusable URL. The normalized code seeds a reproducible relative task order for the selected task subset. The overview screen shows the normalized code and allows correction before the battery begins.
...
- Battery version: `5.4.0`; trial CSV schema version: `11`.
...
Use [RELEASE_CHECKLIST.md](RELEASE_CHECKLIST.md) and [PRODUCTION_DEPLOYMENT.md](PRODUCTION_DEPLOYMENT.md) before every study release. In addition to the CI result, archive the exact commit/tag, frozen deployment configuration and digest, production origins, application-asset-set and served-script SHA-256 values, approved protocol/consent versions, supported browser/device evidence, selected operating mode, known limitations, exact distributed participant links, external return-portal evidence, and rollback target. [NOTICE.md](NOTICE.md) records the present artifact/right status. Code, source audio, and derivative-stimulus rights must be resolved separately; no audio licence is inferred from repository presence or successful validation.
