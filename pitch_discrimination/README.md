...
The FLAC files are byte-identical to the official OSF `OFFLINE AP TESTS.zip` distribution, but that distribution's 500-ms pitch and approximately 50-ms ramp do not match every cited article specification. See the root `PROCEDURE_AUDIT.md`; do not edit these files in place.
...
- Threshold: mean of reversal Levels from the third reversal onward, shown as both a published Level and an F0 difference; CSV downloads automatically.
...
- `threshold_physical_value` is the F0 difference in the unit given by `threshold_unit`; `schema_version` is `2`.
...
- `1.flac` (Level 0) has F0 = 330 Hz; Levels 1–100 span 330.3–360 Hz.
- Physical threshold: `0.3 × threshold_estimate` Hz F0 difference.
