...
function levelToPhysical(level) {
  return level * TASK_METADATA.levelSize;
}

function formatThreshold(level) {
  return `Level ${level.toFixed(2)} (${TASK_METADATA.thresholdMeasure}: ${levelToPhysical(level).toFixed(2)} ${TASK_METADATA.thresholdUnit})`;
}
...
function handleResponse(choice) {
  ...
  const meanReversal = state.numReversals > 2
    ? state.reversalLevelSum / (state.numReversals - 2)
    : '';
  const prevLevel = fileStepToLevel(prevStep);
  const nextLevel = fileStepToLevel(state.currentStep);

  results.push({
    subject_id: subjectId,
    trial: state.currentTrial + 1,
    stimulus_step: prevLevel,
    stimulus_file_index: prevStep,
    ...
    step_before: prevLevel,
    file_index_before: prevStep,
    ...
    threshold_unit: TASK_METADATA.thresholdUnit,
    schema_version: 2
  });
  ...
  if (state.numReversals === 0) {
    if (prevLastCorrect > -1) {
      if ((prevLastCorrect === 1 && !wasCorrect) || (prevLastCorrect === 0 && wasCorrect)) {
        ...
        if (state.numReversals > 2) {
          state.reversalLevelSum += fileStepToLevel(state.currentStep);
        }
      }
    }
...
function conclude() {
  const threshold = state.numReversals > 2
    ? state.reversalLevelSum / (state.numReversals - 2)
    : null;
  ...
  elements.thresholdText.textContent = threshold !== null
    ? `Estimated threshold (mean of reversals 3 onward): ${formatThreshold(threshold)}`
...
function downloadCsv() {
  const header = ['subject_id', 'trial', 'stimulus_step', 'stimulus_file_index', 'odd_position', 'correct_answer', 'response', 'correct', 'rt_ms', 'num_reversals_after', 'step_before', 'file_index_before', 'step_after', 'file_index_after', 'step_size_used', 'mean_reversal_so_far', 'threshold_estimate', 'threshold_physical_value', 'threshold_unit', 'schema_version'];
